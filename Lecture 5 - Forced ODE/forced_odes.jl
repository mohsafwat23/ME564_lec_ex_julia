...
# ╔═╡ f5ae391a-79d1-11ed-30d5-2378c7edb6ca
using LinearAlgebra, ControlSystems, Plots

# ╔═╡ 753463a5-7e7b-4417-a433-1e6a71c3d06a
md"""
## Linear Differential Equations with Forcing
$$\dot x = Ax + Bu$$
$$y = Cx + Du$$
where $x$ is the internal state, $u$ is the control input, and $y$ is the output/measurment/observation
"""

# ╔═╡ 7f1b7657-0573-4111-b0ce-14b91ffc25e3
md"""
### Pendulum example with Imulse Response
$$\ddot \theta = -sin(\theta) + \tau$$
where $\tau$ is the control input
Linearize about $\theta = 0$

$$\frac{d}{dt} \begin{bmatrix} \theta \\ \dot \theta \end{bmatrix} = 
\underbrace{\begin{bmatrix} 0 & 1\\ -1 & 0 \end{bmatrix}}_{\text{A}} \begin{bmatrix} \theta \\ \dot \theta \end{bmatrix} + \underbrace{\begin{bmatrix} 0 \\ 1 \end{bmatrix}}_{\text{B}} \tau$$

$$\begin{bmatrix} \hat{\theta} \\ \hat{\dot \theta} \end{bmatrix} = \underbrace{\begin{bmatrix} 1 & 0\\ 0 & 1 \end{bmatrix}}_{\text{C}} \begin{bmatrix} \theta \\ \dot \theta \end{bmatrix}$$
"""

# ╔═╡ 95dc33e6-436a-4704-af16-1a4fef5f8f43
begin
	A = [0 1; -1 -0.1]  #added some damping 0.1
	B = [0; 1]
	C = I(2)
	D = [0; 0];
	sys = ss(A,B,C,D);
	x,t = impulse(sys,100)
	p1 = plot(t,x[1,:], label="θ", ylabel="θ (rad)")
	p2 = plot(t,x[2,:], label = "ω", ylabel="ω (rad/s)")
...
git-tree-sha1 = "48de8e16707810662e82df2b3c770c0baa0a9e6e"

...
# ╔═╡ 531ed7e5-d6f8-4088-b0d1-a0b163d7e014
using CairoMakie, ODE

# ╔═╡ 8b72d5a2-ab81-4514-bdca-a64c22acc006
md"""
## 6th order Potential Function
$$P(x) = (x^6 - 12*x^4 + 36*x^2)/12$$

#### Newton's Second Law
$$\ddot x = - \frac{\partial P}{\partial x}=-0.5x^5 + 4x^3 - 6x - \dot x$$
where $\dot x$ is added friction
"""

# ╔═╡ 962b7072-66fd-4314-a3b0-6b5fc2653d6c
begin	
	 # x'(t) = y, y'(t) = -0.5x^5 + 4x^3 - 6x - x'
	odeSol(x, y) = Point2f(y, -0.5*x^5 + 4*x^3 -6*x - y)
	fig = Figure(resolution = (800, 600))
	ax = Axis(fig[1, 1], xlabel = "x", ylabel = "y", backgroundcolor = :black)
	streamplot!(ax, odeSol, -4 .. 4, -4 .. 4, colormap = Reverse(:plasma),
	    gridsize = (40, 40), arrow_size = 10)
	fig
end

# ╔═╡ a2952a1a-ebb9-45ba-9cdb-7505820cafc5
md"""
## Pendulum
$$\ddot \theta = -\sin(\theta)$$
...
# ╔═╡ a78a996c-e6d5-4e7c-a750-46bc89a272f4
begin	
	 # x'(t) = y, y'(t) = -sin(x) + optional friction 
	odeSolPend(x, y) = Point2f(y, -sin(x))
	figPend = Figure(resolution = (800, 600))
	axPend = Axis(figPend[1, 1], xlabel = "x", ylabel = "y", backgroundcolor = :black)
	streamplot!(axPend, odeSolPend, -4 .. 4, -4 .. 4, colormap = Reverse(:plasma),
	    gridsize = (40, 40), arrow_size = 10)
	figPend
end
